...
def get_tr_flops(net, input_shape):
    ...
    try:
        ...
        stage5 = sra_flops(H // 32, W // 32, decode_net.attn4.attn.pool_ratio,
                               decode_net.attn4.attn.dim1,decode_net.attn4.attn.num_heads )

        stage6 = sra_flops(H // 16, W // 16,decode_net.attn3.attn.pool_ratio,
                               decode_net.attn3.attn.dim1,decode_net.attn3.attn.num_heads)
        stage7 = sra_flops(H // 8, W // 8,decode_net.attn2.attn.pool_ratio,
                               decode_net.attn2.attn.dim1,decode_net.attn2.attn.num_heads)
    except:
        stage1 = sra_flops(H // 4, W // 4,
    ...
    flops += stage1 + stage2 + stage3 + stage4 + stage5 * level1_layer + stage6 * level2_layer + stage7 * level3_layer
